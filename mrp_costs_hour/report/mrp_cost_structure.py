...
class MrpCostStructure(models.AbstractModel):
	...
	def get_lines(self, productions):
		...
		for product in productions.mapped('product_id'):
			...
			if Workorders:
				query_str = """SELECT w.operation_id, op.name, partner.name, sum(t.duration), wc.currency_id, wc.costs_hour
								...
								GROUP BY w.operation_id, op.name, partner.name, t.user_id, wc.currency_id, wc.costs_hour
								ORDER BY op.name, partner.name
							"""
				company_id = self.env.user.company_id
				self.env.cr.execute(query_str, (tuple(Workorders.ids), ))
				for op_id, op_name, user, duration, currency_id, cost_hour in self.env.cr.fetchall():
					...
					operations.append([
						...
						op_name,
						duration / 60.0,
						currency._convert(cost_hour, company_id.currency_id, company_id, fields.datetime.today())
					])

...
class ValidationDocument(models.Model):
        ...
        if valor:
            if nationality == 'V' or nationality == 'E':
                if len(valor) in range(5,9):
                    ...
                    raise exceptions.except_orm(('Advertencia!'),
                                                         (u'La Cedula de Identidad no puede ser menor que 5 cifras ni mayor a 8.'))
            if nationality == 'P':
    ...
    #                 found = False
    #     return found
    def validate_ci_duplicate(self, valor, parent_id):
        found = True
        partners = self.search([('identification_id', '=', valor), ('parent_id', '=', False)])
        for partner in partners:
            if not parent_id or parent_id != partner.id:
                found = False
            else:
                found = True
                break
    ...
    #     nationality = vals.get('nationality', False)
    #     if identification_id or nationality:
    #         self.validation_document_ident(
    #             identification_id or self.identification_id,
    #             nationality or self.nationality
    #         )
    #     if identification_id and not self.validate_ci_duplicate(identification_id):
    #         raise exceptions.except_orm(('Advertencia!'),
    ...
    def write(self, vals):
        ...
        if identification_id and not self.validate_ci_duplicate(identification_id, parent_id):
            raise exceptions.except_orm(('Advertencia!'),
                                        (u'El cliente o proveedor ya se encuentra registrado con el Documento: %s') % (
                                            vals.get('identification_id', False)))
        res = super(ValidationDocument, self).write(vals)
    ...
    def create(self, vals):
        ...
        if vals.get('identification_id') and vals.get('nationality'):
            ...
            self.validation_document_ident(valor, nationality)
        if vals.get('identification_id'):
            if not self.validate_ci_duplicate(vals.get('identification_id', False), vals.get('parent_id', False)):
                raise exceptions.except_orm(('Advertencia!'),
                                            (u'El cliente o proveedor ya se encuentra registrado con el Documento: %s') % (
                                                vals.get('identification_id', False)))
        res = super(ValidationDocument, self).create(vals)

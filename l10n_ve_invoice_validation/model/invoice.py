...
#                                 states={'draft': [('readonly', False)]},
    ...
    def create(self, vals):
        ...
        if vals.get('invoice_origin'):
            ...
            partner_obj = self.env['res.partner'].search([('id', '=', partner_id)], limit=1)
            hola = self.env['purchase.order'].search([('name','=',vals.get('invoice_origin'))], limit=1)
            if hola:
    ...
    def write(self, vals):
        ...
        for move_account in self:
            if move_account.type == 'in_invoice' and move_account.invoice_origin:
                order_purchase = self.env['purchase.order'].search([('name', '=', self.invoice_origin)], limit=1)
                if order_purchase:

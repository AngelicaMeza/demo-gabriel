...
class WizardReport_2(models.TransientModel): # aqui declaro las variables del wizar que se usaran para el filtro del pdf
        ...
        for det in cursor_resumen:
            for det_line in det.act_code_ids:
                values={
                    ...
                    'factura_total': abs(det.invoice_id.amount_total_signed),
                    'base_imponible': abs(det_line.base_tax),
                    'retenido': abs(det_line.wh_amount),
    ...
    def print_resumen_municipal(self):
        #pass
        self.get_invoice()
        return {'type': 'ir.actions.report','report_name': 'l10n_ve_withholdings_summary.libro_resumen_municipal','report_type':"qweb-pdf"}
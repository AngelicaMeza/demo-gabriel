...
class TxtIva(models.Model):
    ...
    def get_document_number(self, txt_line, inv_type):
        ...
        if txt_line.invoice_id.type in ['in_invoice', 'in_refund']:
            if not txt_line.invoice_id.supplier_invoice_number:
                raise exceptions.except_orm(
                    _('Acción Invalida!'),
                    _("No se puede hacer el archivo txt porque la factura no tiene número de referencia gratis!"))
            else:
    ...
    @api.model
    def get_amount_line(self, txt_line, amount_exempt, alicuota):
        ...
        total = (txt_line.untaxed + ((txt_line.untaxed * alicuota)/100))
...
class TxtIvaLine(models.Model):
    ...
    tax_wh_iva_id = fields.Char( string='Líneas de impuesto de Retención de IVA', readonly=True)

...
class StockDeliveryOrder(models.AbstractModel):
	_name = 'report.stock_reports.delivery_order'
	_description = 'Stock delivery report'

	@api.model
	def _get_report_values(self, docids, data=None):
		docs = self.env['stock.picking'].browse(docids)

		not_allowed = docs.filtered(lambda p: p.picking_type_code != 'outgoing' or p.state not in ['assigned','done'])
		if not_allowed:

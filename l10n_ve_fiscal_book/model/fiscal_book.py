...
class FiscalBook(models.Model):
   ...
   # company_id = fields.Many2one('res.company', 'Compañia', help='Company', )
        ...
        for fb_brw in local_fb:
            ...
            for lin in self.fbl_ids:
                ...
                for lin2 in self.fbl_ids:
                    if lin.doc_type == lin2.doc_type and lin.emission_date == lin2.emission_date and lin.ctrl_number == lin2.ctrl_number and lin.invoice_number == lin2.invoice_number and lin.wh_number == lin2.wh_number:
                        condi += 1
                if condi > 1:
                    lin.unlink()
    ...
    def order_book_lines(self, fb_id):
        """ It orders book lines by a set of criteria:
            - chronologically ascendant date (For purchase book by emission date, for sale book by accounting date).
    ...
    def update_book_lines(self, fb_id):
        ...
        # Recorrido de account_move para evaluar cada documento (factura) que pertenezca a ese libro fiscal

        for inv_brw in self.browse(fb_id).issue_invoice_ids:
           
            

            """Si la factura tiene retención de IVA:
                - Evalúa:
                1) Si el estado de la retención de la factura está en "borrador" o "hecho" (account_wh_iva)
                2) Si la factura base está en estado "Publicado" ó "Cancelado" (account_move)
                3) Si NO es excento de impuestos

                Dependiendo de la primera condición, el registro se asignará ya sea a inv_siva ó inv_iva

                inv_siva faturas con retención en estado borrador
                inv_iva facturas con retención en estado hecho
            """

            if inv_brw.wh_iva_id.state == 'draft' and (inv_brw.state == 'posted' or inv_brw.state == 'cancel') and inv_brw.sin_cred == False:
                inv_siva += inv_brw
            elif inv_brw.wh_iva_id.state == 'done' and (inv_brw.state == 'posted' or inv_brw.state =='cancel') and inv_brw.sin_cred == False:
                inv_iva += inv_brw
            
            """Sino
                - Evalúa:
                1) Si la factura NO tiene retención de IVA (account_wh_iva)
                2) Si el partner (cliente/proveedor) NO es agente de retención (res_partner)
                3) Si la factura base está en estado "Publicado" ó "Cancelado" (account_move)
                4) Si NO es excento de impuestos

                Dependiendo de la segunda condición, el registro se asignará ya sea a inv_siva ó inv_iva

                inv_siva faturas con partner que es agente de retención
                inv_iva facturas con partner que NO es agente de retención
            """
        ...
        if inv_iva:
            """Entra en el condicional si la factura tiene retencion con estado hecho
            ó si no tiene retención pero el partner no es agente de retención"""
            ...
            for iwdl_brw in iwdl_ids:
                    ...
                    if (self.get_doc_type(inv_id=iwdl_brw.invoice_id.id, iwdl_id=iwdl_brw.id) == 'N/C') :
                    ...
                    selectedinvoice = iwdl_brw.invoice_id.id
                    ctrl_number = iwdl_brw.invoice_id.nro_ctrl
                    local_period = self.get_time_period(self.time_period)
                    invoi_date= iwdl_brw.invoice_id.date or False
                    if local_period.get('dt_from') <= iwdl_brw.retention_id.period_id <= local_period.get('dt_to') and local_period.get('dt_from') <= iwdl_brw.invoice_id.date <= local_period.get('dt_to'):
                        iva_rate=iwdl_brw.wh_iva_rate
                        wh_numberr= iwdl_brw.retention_id.number
                        get_wh_vatt = iwdl_brw and iwdl_brw.amount_tax_ret or 0.0
                        
                    elif (iwdl_brw.retention_id.period_id < local_period.get('dt_from') or iwdl_brw.retention_id.period_id > local_period.get('dt_to')) and local_period.get('dt_from') <= iwdl_brw.invoice_id.date <= local_period.get('dt_to'):
                        iva_rate=''
                        wh_numberr= ''
                        get_wh_vatt = ''
                    elif local_period.get('dt_from') <= iwdl_brw.retention_id.period_id <= local_period.get('dt_to') and (iwdl_brw.invoice_id.date < local_period.get('dt_from') or iwdl_brw.invoice_id.date > local_period.get('dt_to')):
                        debit_codicional=''
                        credit_codicional= ''
                        affected_invoice= invoice_number
                        selectedinvoice = ''
                        iva_rate=iwdl_brw.wh_iva_rate
                        wh_numberr= iwdl_brw.retention_id.number
                        get_wh_vatt = iwdl_brw and iwdl_brw.amount_tax_ret or 0.0
                        ctrl_number = ''
                        invoice_number = ''
                        invoi_date = iwdl_brw.retention_id.period_id
                        



                    # line.iwdl_id.retention_id.date_ret if line.wh_number 
                    values = {
                                'iwdl_id': iwdl_brw.id,
                                'type': t_type,
                                'accounting_date': iwdl_brw.invoice_id.invoice_date or False, 
                                'emission_date': invoi_date,
                                ...
                                'credit_affected':  credit_codicional, 
        ...
        for inv_otro in inv_siva:
                """Recorrido de las facturas que tienen retencion con estado borrador
                ó si no tiene retención pero el partner si es agente de retención"""
    ...
    def create_consolidate_line(self, fb_id, line_tuple):
        ...
        fbl_obj = self.env['fiscal.book.line']
        float_colums = ['total_with_iva', 'vat_exempt',
    ...
    def update_book_taxes_summary(self):
        """ It updates the summaroty of taxes by type for this book.
        @param fb_id: fiscal book id
        """
        self.clear_book_taxes_summary()
        tax_types = ['exento', 'reducido', 'general', 'adicional']
  ...
  #      ait_obj = self.env['account.invoice.tax']
        ...
        data['do_sdcf_and_exempt_sum'] = self.type == 'sale' \
                                         and (data['tp_exempt_vat_sum'] +
                                              data['ntp_exempt_vat_sum']) \
                                         or (data['do_exempt_vat_sum'])
        ...
        data['imex_vat_base_sum'] += data['imex_exempt_vat_sum']
    ...
    def update_vat_fields(self, field_name):
        ...
        tax_type = {'exempt': 'exento', 'reduced': 'reducido',
 ...
 #       ait_obj = self.env['account.invoice.tax']
        ...
        for fbl in fiscal_book.fbl_ids:
            ...
            if fbl.report_z_id:
                ...
                amount_field_data = {'total_with_iva': 0.0, 'vat_exempt': 0.0, 'vat_general_base': 0.0, }

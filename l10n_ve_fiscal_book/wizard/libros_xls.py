...
class FiscalBookWizard(models.TransientModel):
    ...
    def xls_compra(self):
        ...
        for datos in datos_compras:
            ...
            writer.write_merge(row, row, 54, 55, datos.get('date_wh_number'), sub_header_style_1)
    ...
    def xls_ventas(self):
        ...
        for line in fbl_obj:
            if line:
                ...
                date_wh_number = datetime.strftime(datetime.strptime(str(line.iwdl_id.retention_id.date_ret), DEFAULT_SERVER_DATE_FORMAT), format_new) if line.get_wh_vat or line.wh_number else '',
                ...
                #             'report_z': '',
                #             'export_form': '',
                #             'wh_number': wh_numberr if wh_numberr else '',
                ...
                docs.append({
                    ...
                    'vat_general_rate': str(valor_alic_general) if valor_alic_general else '0' , #+ '  ' + str(vat_reduced_rate) + ' ' + str(vat_additional_rate) + '  ',
        ...
        for datos in docs:
            ...
            writer.write_merge(row, row, 22, 23, [f"{'{:.2f}'.format(datos.get('total_w_iva'))}"]  , sub_header_style_1)
            ...
            writer.write_merge(row, row, 28, 29, [f"{'{:.2f}'.format(datos.get('vat_general_base'))}"], sub_header_style_1)
            if datos.get('vat_general_rate') != '':
                writer.write_merge(row, row, 30, 31, datos.get('vat_general_rate') + '%', sub_header_style_1)
            writer.write_merge(row, row, 32, 33, [f"{'{:.2f}'.format(datos.get('vat_general_tax'))}"], sub_header_style_1)
            ...
            writer.write_merge(row, row, 46, 47, [f"{'{:.2f}'.format(datos.get('get_wh_vat'))}"], sub_header_style_1)
    ...
    def obtener_tasa(self, invoice):
        ...
        tasa = self.env['res.currency.rate'].search([('currency_id', '=', tasa_id.id), ('write_date', '<=', fecha)], order='id desc', limit=1)
        if not tasa:
